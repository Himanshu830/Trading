...
const UpdateProduct = ({ user, token, match}) => {
    ...
    const getCategoryList = async (parent = null) => {
        getCategories(token, parent).then(data => {
            if (!data) {
                ...
                setProduct({ ...product, categories: data.result });
            }
        });
    };

    const getProduct = () => {
        return getProductById(productId, token).then(response => response)
    }
        ...
        .then(response => {
            getCategories(token).then(data => {
                if (!data) {
                    ...
                    setProduct({ ...product, ...response.result, categories: data.result, formData: new FormData() });
                }
            })
        })
    }, [])
                            ...
                            <form>
                                ...
                                <div className="form-row">
                                    <div className="form-group col-md-6">
                                        ...
                                        <select onChange={handleChange('categoryId')} className="form-control" defaultValue={categoryId}>
